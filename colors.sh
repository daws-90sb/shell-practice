...
VALIDATE(){
    if [ $2 -ne 0 ]; then
        echo  " $TIME_STAMP [ERROR]  Installing $1 ...... $R FAILED! $N " | tee -a $LOG_FILE  
        exit 1
    else
        echo " $TIME_STAMP [INFO] Installing $1 ......$G SUCCEDED! $N" | tee -a $LOG_FILE
     fi
}
...
do 
  ...
  if [ $? -ne 0 ]; then
    ...
    echo " $TIME_STAMP [INFO] $package already installed ....$Y SKIPPING $N"
